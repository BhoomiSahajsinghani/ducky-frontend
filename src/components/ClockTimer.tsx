...
export default function ClockTimer({ initialMinutes = 25, editable = true, onMinutesChange }: ClockTimerProps) {
  ...
  const handleEditBlurOrEnter = () => {
    ...
    if (match) {
      const newMinutes = Math.min(MAX_MINUTES, Math.max(MIN_MINUTES, parseInt(match[1])));
      const newSeconds = match[2] ? Math.min(59, Math.max(0, parseInt(match[2]))) : 0;
      setMinutes(newMinutes);
      setSeconds(newSeconds);
      if (onMinutesChange) onMinutesChange(newMinutes);
    }
    setEditing(false);
  };
